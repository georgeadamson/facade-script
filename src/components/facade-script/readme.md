...
| `ready`                | --          | A function that will return true when your script has loaded an run. For example to detect `'myVideoPlayer' in window`. Without this we assume the script is ready for use as soon as it loads. | `Function`                                                                                                      | `undefined` |
...
| `srcProd` _(required)_ | `src`       | Required. src for the `<script>` or `<iframe>` that will be added to the DOM when lazyload is triggered.                                                                                        | `string`                                                                                                        | `undefined` |
...
| `wait`                 | `wait`      | Delay n milliseconds after being triggered. Defaults to no wait                                                                                                                                 | `number`                                                                                                        | `undefined` |
